# explorative data analysis, course project2: plot2.R
...
png("plot2.png")
#plot w/o axis
plot(emissions_baltimore$year, emissions_baltimore$Emission_sum, type="b", xaxt="n", yaxt="n", 
     xlab="Year", ylab="Pm 2.5 emission in [t]")

# x-axis
axis(1, at = emissions_baltimore$year, labels = emissions_baltimore$year)

# y-axis
axis(2, at = pretty(emissions_baltimore$Emission_sum), 
     labels = format(pretty(emissions_baltimore$Emission_sum), big.mark = ","))
dev.off()

# Answer: Pm 2.5 emissions in Baltimore City fell from 3500t in 1999 to under 2000t in 2008. Keep in mind that pm2.5 is only one kind of emission and there are several others, that are maybe rising (like CO2).